...
class UserDataController extends ChangeNotifier {
  ...
  void initUser() async {
    ...
    // if (Hive.box("configuration").get("mnemonic"))
    String mnemonic = Hive.box("configuration").get("mnemonic");
    final stringKey = await pdfk2.deriveKey(secretKey: SecretKey(mnemonic.codeUnits), nonce: []);
    final secretKeyData = await stringKey.extract();
    debugPrint("Key: ${ConvertPack.base64Encode(secretKeyData.bytes)}");
    debugPrint("Ky2: ${keyGen.genFortuna()}");
  ...
  void onPrivateContentRetrieve(content) {
    if (content.asString == null) {
      ...
      AesCrypt encryption = AesCrypt(key: mnemonicToEntropy(configBox.get("mnemonic")), padding: PaddingAES.pkcs7);
  ...
  void onContentRetrievalFailure(error) async {
    ...
    if (error.toString() == "Exception: not found") {
      ...
      AesCrypt encryption = AesCrypt(key: mnemonicToSeedHex(configBox.get("mnemonic")), padding: PaddingAES.pkcs7);
      ...
      await skynetClient.skydb
          .setFile(
              ...
              SkyFile(
                  ...
                  type: "text/plain"))
          .then((value) => debugPrint("$value file set"))
          .onError((error, stackTrace) => debugPrint("${error.toString()} ${stackTrace.toString()}"));
    } else {
